...
from MIIL_DETECTION_DIRECTION2_CV_GUI import Ui_MainWindow #QT Designerで作成し変換したファイルの読込
        ...
        #!!!!!!!!!!openCVの処理は此処で行う!!!!!!!!!!
            if capLoop == 1: #ループモードの場合
                ...
                if win.ui.checkBox4.isChecked() == False:
                    classes, scores, boxes = model.detect(frameB , float(win.ui.comboBox1.currentText()), 0.4)
                    if len(scores) == 0: #対象物を検出しなかった場合は出力をオフにする
                        outPut = 0
                        end = time.time()
                        fps = int(1 / (end - start))
                        print("[DETECTED] : NONE    [CONFIDENCE] : NONE    [OUTPUT] : " + str(outPut) +"    [FPS] : " + str(fps) ) #ラベルと信頼度をコンソールに表示
                    for (classid, score, box) in zip(classes, scores, boxes): #検出リスト配列から各検出を取得
                        LABEL = class_names[classid] #ラベル名を取得
                        if(LABEL in label_color) == False: #ラベル名がラベル色保存用ディクショナリにないか確認
                            ...
                            if color_pos == 6: #6パターン毎に色の明度を下げる
                                color_pos = 0
                                color_code -= 10
                        CONFIDENCE = str(int(score * 100)) #検出物の信頼度を格納
                        ...
                        fps = int(1 / (end - start))
                        print("[DETECTED] : " + LABEL + "    [CONFIDENCE] : " + CONFIDENCE + "    [OUTPUT] : " + str(outPut) +"    [FPS] : " + str(fps) ) #ラベルと信頼度をコンソールに表示
                else:
                    cvKey = cv2.waitKey(1)
...
#=====GUI用クラス継承の定型文========================================
    def __init__(self, parent = None): 
        ...
        self.ui.comboBox8.addItems(["32", "64", "96", "128", "160", "192", "224", "256", "288", "320", "352", "384", "416", "448", "480", "512", "544", "576", "608"]) #####コンボボックスにアイテムを追加
        self.ui.comboBox8.setCurrentIndex(9) #コンボボックスのアイテムを選択
        #-----シグナルにメッソドを関連付け----------------------------------------
...
#=====ウィジットのシグナル処理用メッソド========================================
    ...
    def pushButton1_clicked(self):
        ...
        if self.ui.lineEdit3.text() == '' and self.ui.checkBox4.isChecked() == False:
            ...
            ret = msgbox.exec()
        elif self.ui.lineEdit4.text() == '' and self.ui.checkBox4.isChecked() == False:
            msgbox = QtWidgets.QMessageBox(self)
            msgbox.setWindowTitle("MDC")
            msgbox.setText("PLEASE SET MODEL FILE.")
            ret = msgbox.exec()
        elif self.ui.lineEdit5.text() == '' and self.ui.checkBox4.isChecked() == False:
            msgbox = QtWidgets.QMessageBox(self)
            msgbox.setWindowTitle("MDC")
            ...
            if self.ui.checkBox4.isChecked() == True:
                ...
                with open(self.ui.lineEdit3.text(), "r") as f:
                    class_names = [cname.strip() for cname in f.readlines()]
                net = cv2.dnn.readNet(self.ui.lineEdit5.text(), self.ui.lineEdit4.text())
                net.setPreferableBackend(cv2.dnn.DNN_BACKEND_DEFAULT)
                net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)
                model = cv2.dnn_DetectionModel(net)
                DET_SIZE = self.ui.comboBox8.currentText()
                DET_SIZE =int(DET_SIZE)
                model.setInputParams(size=(DET_SIZE, DET_SIZE), scale=1/255, swapRB=True)
                self.ui.pushButton1.setEnabled(False)
                self.ui.pushButton2.setEnabled(True)
                self.ui.pushButton3.setEnabled(False)
                self.ui.pushButton4.setEnabled(False)
